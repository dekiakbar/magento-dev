...
# replace existing cert with new domain certificate
...
openssl req -x509 -newkey rsa:2048 -sha256 -days 730 -nodes \
  ...
  -subj "/C=US/ST=TX/L=Austin/O=Adobe Commerce/OU=Cloud Docker/CN=${MAGENTO_DOMAIN}" \
  -addext "subjectAltName=DNS:${MAGENTO_DOMAIN},DNS:www.${MAGENTO_DOMAIN}"
